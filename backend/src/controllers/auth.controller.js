...
export const signup = async (req, res) => {
    ...
    try{
        ...
        if (userAlreadyExists) {
            // Unverified account with correct password → resend verification instead of erroring
            if (!userAlreadyExists.isVerified) {
                const passwordMatches = await bcryptjs.compare(password, userAlreadyExists.password);
                if (passwordMatches) {
                    ...
                    try {
                        await sendVerficationEmail(userAlreadyExists.email, verificationToken);
                    } catch (emailErr) {
                        console.error('Error resending verification email:', emailErr);
                    }
                    generateTokenAndSetCookie(res, userAlreadyExists._id, userAlreadyExists.isAdmin);
                    return res.status(200).json({
                        success: true,
                        message: 'Verification email resent. Please check your inbox.',
                        user: { ...userAlreadyExists._doc, password: undefined },
                    });
                }
            }
